...
function createBoundingBoxShape (object) {
  console.log("createBoundingBoxShape",object)
...
function getGeometry(object) {
  ...
  while ((mesh = meshes.pop())) {
    ...
    if (geometry.isBufferGeometry && geometry.attributes.position.itemSize > 2) {
      combined.merge(geometry, mesh.matrixWorld);
    } else {
      var tmpGeometry = new THREE.BufferGeometry().fromGeometry(geometry);
      tmpGeometries.push(tmpGeometry);
      tmpGeometry.applyMatrix(mesh.matrixWorld);
    }
  }

  tmpGeometries.forEach((geometry) => {
